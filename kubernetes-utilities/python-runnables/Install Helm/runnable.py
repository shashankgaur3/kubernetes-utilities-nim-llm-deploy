    ...
    def run(self, progress_callback):
        ...
        if not os.path.exists(self.tmp_dir):
            process = subprocess.Popen(
                ["curl", "-fsSL", "-o", self.data_dir+"/tmp/helm/get_helm.sh", "https://raw.githubusercontent.com/helm/helm/main/scripts/get-helm-3", "--create-dirs"],
        ...
        if not os.path.exists(self.helm):
            ...
            process = subprocess.Popen(
                ["{}/helm".format(bin_dir), "repo", "add", "stable", "https://charts.helm.sh/stable"],
                stdout=subprocess.PIPE,
                stderr=subprocess.PIPE
            )
            stdout, stderr = process.communicate()
